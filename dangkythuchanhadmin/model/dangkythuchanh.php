<?php			
		...
		elseif (isset($_GET['id'])) {
			...
			$result = array();
			$mand=$_SESSION["mand"];
			$rs = $conn->query("SELECT COUNT(1) FROM view_dangkyphonthuhanh ");	
			$row = mysqli_fetch_row($rs);
			$result["total"] = $row[0];
			$s="SELECT dangkyphongthuchanh.ID,  tennd, tenphong, tenmon, buoi, tiet, hocky, namhoc, ngaydangky, ngaytraphong, dangkyphongthuchanh.ghichu FROM `dangkyphongthuchanh`JOIN `nguoidung` on dangkyphongthuchanh.mand=nguoidung.mand JOIN phong on dangkyphongthuchanh.maphong = phong.maphong join monhoc on dangkyphongthuchanh.mamon = monhoc.mamon  
				limit $offset,$rows";	
			$rs = $conn->query($s);
				$items = array();
			while($row = mysqli_fetch_object($rs)){
