...
if(isset($_POST['btnGui'])){
	...
	$name=$listWorkSheets;
		...
		$mand=$_SESSION["mand"];
		$sql = "INSERT INTO `lichhoclythuyet`(`ID`, `mand`, `malophp`, `sisolhp`, `mamon`, `thu`, `sotietlt`, `sotietth`, `tiet`, `ngaybd`, `ngaykt`, `hocky`, `nienkhoa`, `malop`, `phong`) VALUES ('','$mand','$nhom','$sisolhp','$mamon','$thu','$sotietlt','$sotietth','$tiet', STR_TO_DATE('$ngaybd', '%m/%d/%Y'), STR_TO_DATE('$ngaykt', '%m/%d/%Y'),'$hocky','$nienkhoa','$malop','$phong')";
		$thucthi=$conn->query($sql);
		if ($thucthi) {
			
			header("Location:../lichhoclythuyetnguoidung/view/index.php");
		}
		 // header("Location:../lichhoclythuyetnguoidung/view/index.php");
	}
