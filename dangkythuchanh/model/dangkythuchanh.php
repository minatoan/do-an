<?php			
		...
		elseif (isset($_GET['id'])) {
			...
			if($tungay !="" or $denngay !=""){
				...
				$rs = $conn->query("SELECT COUNT(1) FROM view_dangkyphonthuhanh WHERE 1".$nienkhoa.$hocky.$malophp.$ngaythang.$maphong.$buoi);	
				$row = mysqli_fetch_row($rs);
				$result["total"] = $row[0];
				$s="SELECT view_dangkyphonthuhanh.*,(SELECT nguoidung.tennd FROM nguoidung WHERE nguoidung.mand = view_dangkyphonthuhanh.mand LIMIT 0,1) as tennd,date_format(view_dangkyphonthuhanh.ngaydangky,'%w') as thu FROM view_dangkyphonthuhanh WHERE 1".$nienkhoa.$hocky.$malophp.$ngaythang.$maphong.$buoi." limit $offset,$rows";	
				$rs = $conn->query($s);
					$items = array();
				while($row = mysqli_fetch_object($rs)){
						array_push($items, $row);
			...
			$row = mysqli_fetch_row($rs);
			$result["total"] = $row[0];
			$s="SELECT view_dangkyphonthuhanh.* FROM view_dangkyphonthuhanh WHERE view_dangkyphonthuhanh.mand='$mand'
				limit $offset,$rows";	
			$rs = $conn->query($s);
				$items = array();
			while($row = mysqli_fetch_object($rs)){
